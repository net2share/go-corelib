...
type BinaryDef struct {
	...
	Archive       bool                     // true if download is tar.xz (kept for backward compat)
	ArchiveType   string                   // archive type: "", "tar.xz", "zip" (overrides Archive when set)
...
// IsArchive returns true if the binary is distributed as an archive.
func (d BinaryDef) IsArchive() bool {
	if d.ArchiveType != "" {
		return true
	}
	return d.Archive
...
// GetArchiveType returns the archive type for extraction dispatch.
func (d BinaryDef) GetArchiveType() string {
	if d.ArchiveType != "" {
